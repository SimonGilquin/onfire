...
// in simulator we redefine to format for OnFire Developer console
...
{\
	NSString * exceptionName = [@"org.onfire." stringByAppendingString:NSStringFromClass([self class])];\
...
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.onfire";\
	NSString * message = [NSString stringWithFormat:@"%@. %@ %@",reason,(subreason!=nil?subreason:@""),(location!=nil?location:@"")];\
	NSLog(@"[ERROR] %@",message);\
	if ([NSThread isMainThread]==NO) {\
		@throw [NSException exceptionWithName:exceptionName reason:message userInfo:nil];\
...
enum {
	UIOnFireNativeItemNone = -1, 
	UIOnFireNativeItemSpinner = -2,
	UIOnFireNativeItemProgressBar = -3,
	
	UIOnFireNativeItemSlider = -4,
	UIOnFireNativeItemSwitch = -5,
	UIOnFireNativeItemMultiButton = -6,
	UIOnFireNativeItemSegmented = -7,
	
	UIOnFireNativeItemTextView = -8,
	UIOnFireNativeItemTextField = -9,
	UIOnFireNativeItemSearchBar = -10,
	
	UIOnFireNativeItemPicker = -11,
	UIOnFireNativeItemDatePicker = -12,
	
	UIOnFireNativeItemInfoLight = -13,
	UIOnFireNativeItemInfoDark = -14,
	
	UIOnFireNativeItemDisclosure = -15,
	
	UIOnFireNativeItemContactAdd = -16
};
